...
if [[ ${#BACKENDS[@]} -eq 0 ]]; then
  echo "No backends found in scripts/arbitrator/" >&2
  exit 1
fi
...
for backend in "${BACKENDS[@]}"; do
  ...
  export ARBITRATOR_BACKEND="$backend"

  # Isolated environment per backend
  TEST_DIR=$(mktemp -d)
  export ARBITRATOR_SESSION_BASE="${TEST_DIR}/sessions"
  export ARBITRATOR_DB_PATH="${TEST_DIR}/test.db"
