#!/usr/bin/env python3
"""Unified storage-provider for the dev-team pipeline.
...
def dispatch(resource, action, flags, backend=None):
    ...
    if backend is None:
        backend = os.environ.get("STORAGE_PROVIDER_BACKEND",
                    os.environ.get("ARBITRATOR_BACKEND",
                    os.environ.get("PROJECT_STORAGE_BACKEND", "markdown")))
